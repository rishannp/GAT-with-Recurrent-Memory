...
class GATGRU(nn.Module):
    def __init__(self, in_channels, gat_out_channels, gru_hidden_size, num_classes, num_heads, num_gru_layers):
        ...
        # GRU module
        self.gru = nn.GRU(input_size=gat_out_channels, 
                          hidden_size=gru_hidden_size, 
                          num_layers=num_gru_layers, 
                          batch_first=True, dropout=0.5, bidirectional=True)

        # Fully connected layer - adjust input size for bidirectional GRU (hidden_size * 2)
        self.fc = nn.Linear(gru_hidden_size, num_classes)  # multiplied by 2 for bidirectional GRU
    ...
    def forward(self, batch):
        ...
        # Reshape for GRU input (batch_size, sequence_length=1, gat_out_channels)
        graph_embeddings = graph_embeddings.unsqueeze(1)
    
        # Process through GRU
        gru_output, h_n = self.gru(graph_embeddings)
    
        # For single-layer GRU, just use the hidden state from the only layer (h_n[0, :, :])
        final_hidden_state = h_n[0, :, :]
        
        # For multi-layer GRU, use the last. 
        final_hidden_state = h_n[-1, :, :]  # Take the hidden state from the last layer
...
def train_and_evaluate(train_loader, test_loader):
    ...
    gat_out_channels = 32
    gru_hidden_size = 64  # Changed from lstm_hidden_size to gru_hidden_size
    num_heads = 1
    num_gru_layers = 2  # Changed from num_lstm_layers to num_gru_layers
    lr = 1e-3
