...
def train():
    ...
    return end_time - start_time, train_accuracy, epoch_loss, all_w, all_att, all_edge
...
def test(loader):
    ...
    return correct / len(loader.dataset) * 100, inference_time, epoch_loss, all_w, all_att, all_edge
...
for subject_number in subject_numbers:
    ...
    for epoch in range(1, 100):  # Note: Range is 1 to 500 (inclusive)
        epoch_train_time, train_acc, train_loss, trainw3, trainatt3, trainedge3 = train()
        train_accuracies.append(train_acc)  # Store train accuracy

        val_acc, inference_time, val_loss, valw3, valatt3, valedge3 = test(val_loader)
        val_accuracies.append(val_acc)  # Store validation accuracy

        test_acc, inference_time, test_loss, testw3, testatt3, testedge3 = test(test_loader)
        ...
        # Check if validation accuracy has improved, and save the best model
        if val_acc > best_val_acc:
            best_val_acc = val_acc
            best_model_state_dict = model.state_dict()  # Save the model state_dict

            # Optionally, save the model checkpoint to disk
            checkpoint_path = os.path.join(checkpoint_dir, f"best_model_subject_{subject_number}.pth")
            torch.save(best_model_state_dict, checkpoint_path)
            #print(f"Saved best model at epoch {epoch} with val accuracy {best_val_acc:.4f}")

        # Reload the best model after each epoch if it has improved
        if best_model_state_dict is not None:
            model.load_state_dict(best_model_state_dict)
            #print(f"Reloaded best model from epoch {epoch} with val accuracy {best_val_acc:.4f}")

        # Check if test accuracy has improved, and store weights and attention scores
